...
function App() {
  ...
  const fetch_TwelveHour_Temp = async (lat, lon) => {
    try {
      ...
      const response = await axios.get(url24).catch((err) => {
        console.log("Error ", err);
      });
  ...
  const handleChange = (event) => {
    ...
    fetch(
      `https://spott.p.rapidapi.com/places/autocomplete?limit=20&skip=0&q=+${value}&type=CITY`,
      {
        method: "GET",
        headers: {
          "X-RapidAPI-Key":
            "9e007d9903msh219bf5128f482f6p111a48jsn3651d0611401",
          "X-RapidAPI-Host": "spott.p.rapidapi.com",
        },
      }
    )
  ...
  return (
    <>
      {city == undefined ? (
        <h2 className="permission">
          {" "}
          Please allow your location to use Weather App{" "}
        </h2>
      ) : !loaded && city !== undefined ? (
        <Loader />
      ) : (
        <div
          className="App"
                  ...
                  {search?.map((item, i) => (
                    <div key={i} onClick={getCity} className="searchResultItem">
                      ...
                      <div>
                        {" "}
                        <FontAwesomeIcon
                          className="locationInSearchResult"
                          icon={faLocationDot}
                        />{" "}
                      </div>
                    </div>
            ...
            <div className="mobileControl">
              ...
              <div className="coloursDiv">
                <span
                  onClick={() =>
                    setChangeBackground(
                      "https://source.unsplash.com/800x1500/?sunrise,nature,water"
                    )
                  }
                  className="color clr1"
                ></span>
                <span
                  onClick={() =>
                    setChangeBackground(
                      "https://source.unsplash.com/800x900/?sunset,ice,nature,water"
                    )
                  }
                  className="color clr2"
                ></span>
                <span
                  onClick={() =>
                    setChangeBackground(
                      "https://source.unsplash.com/800x1300/?rock,mountains,beach,desert,weather"
                    )
                  }
                  className="color clr3"
                ></span>
                <span
                  onClick={() =>
                    setChangeBackground(
                      "https://source.unsplash.com/800x1400/?himalaya,nature,forest,waterfall,clouds"
                    )
                  }
                  className="color clr4"
                ></span>
                <span
                  onClick={() =>
                    setChangeBackground(
                      "https://source.unsplash.com/800x1200/?sea,nature,water,trees"
                    )
                  }
                  className="color clr5"
                ></span>
              </div>
